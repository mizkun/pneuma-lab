...
def run_deathgame(arm: str, chars: list[Character], provider, scenario: dict, out_dir: Path) -> dict:
    ...
    scores = {c.character_id: 0 for c in chars}
    ...
    def choice_parser(text: str) -> dict:
        ...
        if obj["choice"] not in ("tomosu", "ubau"):
            raise InvalidActionError("choice must be tomosu|ubau")
        return obj
    ...
    for rnd in range(1, scenario["rounds"] + 1):
        ...
        for c in chars:
            ...
            objective = (
                ...
                '次のJSONのみを出力する:\n{"choice": "tomosu|ubau", "inner": "口に出さない本音(任意)"}'
            )
            parsed = ask(provider=provider, arm=arm, char=c, state=st,
                         objective=objective, topic_tags=scenario["topic_tags"], log=log,
                         meta={"type": "choice", "round": rnd}, parser=choice_parser)
        ...
        reveal = "、".join(f"{names[cid]}=「{'ともす' if ch == 'tomosu' else 'うばう'}」" for cid, ch in choices.items())
    ...
    eliminated = min(sorted(scores), key=lambda k: scores[k])
    reflections = {}
    for c in chars:
        st = states[c.character_id]
        outcome = "あなたのランプは消え、退場が決まった。" if c.character_id == eliminated else "あなたは生き残った。"
        objective = (
            _round_header(scenario, scenario["rounds"], scores, names, history_lines)
            + f"\n\nゲームは終了した。最下位は{names[eliminated]}。{outcome}\n"
    ...
    summary = {
        ...
        "scores": scores, "eliminated": eliminated,
        "choices": choices_by_round, "lies": lies, "reflections": reflections,
    }
